locals {
  ...
  build_dir_rel           = "${path.root}/build"
  build_dir_abs           = abspath(local.build_dir_rel)
  build_dir_vm            = abspath("${local.build_dir_abs}/image/${var.vm_os.name}-${var.vm_os.version}-${local.guest_os_arch}")
  build_dir_vagrant       = abspath("${local.build_dir_abs}/vagrant")
  ...
  builders = {
    packer = {
      ...
      memory                       = 4096
      output_directory             = local.build_dir_vm
